export async function onRequestPost(context) {
    try {
        ...
        const prompt = `
作为一位专业的技术文章摘要专家，请为以下文章提供一个全面但简洁的总结（200-250字）。
总结应该：
1. 抓住文章的核心问题和解决方案
2. 提炼出关键技术要点和实操步骤
3. 突出最有价值的信息
...
`;
        ...
        const response = await fetch('https://api.gork.ai/v1/completions', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Gork API错误：${errorData.error?.message || '未知错误'}`);
        }
