export function initClouds(count = 15) {
    ...
    const cloudImages = [
        '/static/images/cloud-image1.webp',
        '/static/images/cloud-image2.webp',
        '/static/images/cloud-image3.webp'
    ];
    ...
    let isCloudAnimationScheduled = false;
    let mouseX = 0;
    let mouseY = 0;
    document.addEventListener('mousemove', (e) => {
        mouseX = e.clientX;
        mouseY = e.clientY;
        if (!isCloudAnimationScheduled) {
            isCloudAnimationScheduled = true;
            requestAnimationFrame(() => {
                ...
                clouds.forEach(cloudWrapper => {
                    ...
                    cloudData.push({
                        wrapper: cloudWrapper,
                        image: cloudWrapper.querySelector('.cloud-image'),
                ...
                cloudData.forEach(data => {
                    ...
                    const deltaX = mouseX - data.centerX;
                    const deltaY = mouseY - data.centerY;
                    ...
                    if (distance < maxDistance) {
                        ...
                        data.image.style.transform = `translate(${moveX}px, ${moveY}px)`;
                    } else {
                        data.image.style.transform = 'translate(0, 0)';
                    }
                });
                isCloudAnimationScheduled = false;
            });
        }
    }, { passive: true });
}
